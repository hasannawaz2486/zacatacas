...
export default function Housing() {
  ...
  return (
    <div
      className="strategic-border row d-flex justify-content-center"
      style={{
    ...
    >
      ...
      >
        <h3 style={{ fontWeight: "bold" }}>
          {" "}
          <span
            style={{ borderBottom: "3px solid #571517", paddingBottom: "5px" }}
          >
            {heading.slice(0, 5)}
          </span>
          {heading.slice(5)}
        </h3>
        <p style={{ marginTop: "20px", fontSize: "14px", marginBottom: "10px" }}>
          {description}
        </p>
        {rentExamples.map((example, index) => (
          <p className="rent-example" key={index}>
            {example.location} <span style={{ fontWeight: "bold" }}>{example.price}</span>
          </p>
        ))}
      </div>
